...
describe SitePrism::Waiter do
  describe '.wait_until_true' do
    ...
    it 'throws a Timeout exception if the block does not become true' do
      expect { execute_waiter(default_timeout) { false } }.to raise_error(SitePrism::Error::TimeoutError).with_message(/#{default_timeout}/)
    end
    ...
    context 'with a custom timeout' do
      ...
      it 'alters the error message' do
        expect { execute_waiter(custom_timeout) { false } }.to raise_error(SitePrism::Error::TimeoutError).with_message(/#{custom_timeout}/)
      end
    end
